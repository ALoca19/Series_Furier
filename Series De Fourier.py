...
def funcion_impar():
    ...
    ao = 0
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion de (-p a p)
    #Por eso se coloca -ls a ls
    ...
    bn = integrate((f) * sin(n * x), (x, 0, ls)) * (2/ls)
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion (-ls,ls)...
    ...
    for i in range(1,t+1): #con un ciclo for de 1 a terminos
        seriebn.append( integrate((f) * sin((i*x*pi)/ls), (x, 0, ls)) * (2/ls) )
        #Se integra con un ciclo para cada termino "n", y se integra de -ls a ls (-p a p)
        #Como indica la formula, la funcion .subs cambia la n por cada uno de los terminos
        #Solo es bn porque es la opcion de senos


    txt.insert(INSERT, "La serie se resolvera usando senos")
    ...
    for i in range(1, t + 1): #con un ciclo for que va de 1 a terminos
        serie = serie + (an*cos(n * x)).subs(n, i) #La formula de an, anque aqui siempre sera 0
    for j in range(1, t + 1): #Otro ciclo de 1 a terminos para bn
        serie = serie + (bn*sin(n * x)).subs(n, j) #Con la fomula para bn
...
def funcion_general():
    ...
    an =  integrate((f) * cos(n * x), (x, li, ls)) * (1/ls)
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion (li,ls)...
    #... Multiplicada por cos((n*pi*x)/pi) como dice la formula para an
    #multiplicamos por 1 sobre L
    
    
    bn = integrate((f) * sin(n * x), (x, li, ls)) * (1/ls)
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion (li,ls)...
    #... Multiplicada por sen((n*pi*x)/pi) como dice la formula para an
    #multiplicamos por 1 sobre L
    ...
    for i in range(1,t+1): #Se usa un ciclo for que va de 1 a terminos, para calcular cada dato
        seriebn.append( integrate((f) * sin((i*x)), (x, li, ls)) * (1/ls) ) 
    ...
    for i in range(1,t+1): #Se usa un ciclo for que va de 1 a terminos, para calcular cada dato
        seriean.append( integrate((f) * cos(i * x), (x, li,ls)) * (1/ls) )
         #Se ingresa a la lista cada termino, que se calcula con la formula, con la funcion .subs se cambia el valor de n...
         #... por cada uno de los terminos y se multiplica por 1/ls como indica la formula. 
    ...
    for i in range(1, t+1): #Con un for vamos a iterar de acuerdo al numero de terminos.
        serie = serie + (an*cos(n * x)).subs(n,i) #Se aplica la formula de series de fourier para an
    for j in range(1, t+1): #Otro for para calcular los terminos de bn
        serie = serie + (bn*sin(n * x)).subs(n,j) #Se aplica la formula de series de fourier para bn
...
def funcion_par():
    ...
    ao = (integrate( f , (x, 0, ls)))*(2/ls)
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion...
    #... en este caso de la opcion cosenos se calcula de (-p a p) por eso de toma -ls y ls
    #multiplicamos por 1 sobre L
    
    #Usamos la funcion pprint para mostrar ao

    an =  integrate((f) * cos(n * x), (x,0, ls)) * (2/ls)
    #integramos la funcion que ingrese el usuario (f) en funcion de x con los limites de integracion (li,ls)...
    #... Multiplicada por cos((n*pi*x)/pi) como dice la formula para an
    #multiplicamos por 1 sobre L
    ...
    for i in range(1,t+1): #Se utiliza un ciclo for de 1 a terminos
        seriean.append( integrate((f) * cos(i*x) , (x, 0, ls)) * (2/ls) )
        #Se integra con un ciclo for para cada termino "n" y los limites son de (-p a p) por eso se pone -ls a ls
        #la funcion .subs cambia la "n" por cada uno de los terminos (1,2,3....n)



    txt.insert(INSERT, "La serie se resolvera usando cosenos")
    ...
    for i in range(1, t + 1): #Con un ciclo for que va de 1 a terminos
        serie = serie + (an*cos(n * x)).subs(n, i) #se le va sumando a la serie cada dato segun la formula
    for j in range(1, t + 1): #Otro for para bn, aunque en este caso siempre sera 0 ya que es la opcion por cosenos.
        serie = serie + (bn*sin(n * x)).subs(n, j)
